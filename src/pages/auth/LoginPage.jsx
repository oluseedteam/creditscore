...
export default function LoginPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try { const user = login(form.email, form.password); navigate(user.role==='admin'?'/admin':'/dashboard') }
    catch (err) { setError(err.message) }
    finally { setLoading(false) }
  }
  const fillDemo = (role) => role==='admin'?setForm({email:'admin@pto.com',password:'admin123'}):setForm({email:'alex@demo.com',password:'demo123'})
    ...
    <div className="min-h-screen flex" style={{background:'#F9F5EF'}}>
      ...
      <div className="hidden lg:flex flex-col justify-between w-5/12 p-12 relative overflow-hidden" style={{background:`linear-gradient(150deg,${TEAL} 0%,${NAVY} 100%)`}}>
        ...
        <Link to="/" className="flex items-center gap-3 relative z-10">
          <div className="w-9 h-9 rounded-xl flex items-center justify-center" style={{background:PGREEN}}>
            <svg width="18" height="18" viewBox="0 0 20 20" fill="none"><path d="M10 2L17 6.5V13.5L10 18L3 13.5V6.5L10 2Z" stroke="white" strokeWidth="1.5" fill="none"/><circle cx="10" cy="10" r="2.5" fill="white"/></svg>
          </div>
        ...
        <div className="relative z-10">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm" style={{color:'#4db2b2'}}>Financial Literacy Program</p>
            </div>
          </div>
        </div>
      ...
      <div className="flex-1 flex items-center justify-center p-8">
        <div className="w-full max-w-md animate-fade-up">
          ...
          <h1 className="font-display text-3xl font-bold mb-1" style={{color:NAVY}}>Welcome back</h1>
          <p className="mb-8" style={{color:'#6B7280'}}>Sign in to continue your journey</p>

          <div className="flex gap-2 mb-6">
            {[['participant','Demo: Participant'],['admin','Demo: Admin']].map(([r,l])=>(
              <button key={r} onClick={()=>fillDemo(r)} className="flex-1 text-xs py-2 px-3 rounded-lg border transition-colors hover:opacity-80" style={{borderColor:'#E3E6EC',color:'#6B7280',background:'white'}}>{l}</button>
            ))}
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <div className="relative">
                <input type={showPass?'text':'password'} required className="input-field pr-12" placeholder="Your password"
          ...
          <p className="text-center text-sm mt-6" style={{color:'#6B7280'}}>
            ...
            <Link to="/signup" className="font-medium transition-colors" style={{color:TEAL}}>Create one</Link>
          </p>
        </div>
      </div>
    </div>
